...
const sellerWorkspaceTabs = [
  { key: "cabinet", label: "Кабинет", Icon: LayoutDashboard },
  { key: "tasks", label: "Задачи", Icon: ClipboardList },
  { key: "reports", label: "Отчеты", Icon: FileText },
  { key: "work-periods", label: "Отчетный период", Icon: CheckSquare },
];
const cardWorkspaceTabs = [
  { key: "card", label: "Карточка", Icon: ShoppingBag },
  { key: "semantic", label: "СЯ", Icon: Tags },
  { key: "audit", label: "Рыночный аудит", Icon: ClipboardList },
  { key: "competitors", label: "Товарный аудит", Icon: Store },
  { key: "changes", label: "Изменения", Icon: WandSparkles },
];
const ozonCardWorkspaceTabs = [
  ...cardWorkspaceTabs,
  { key: "technical", label: "Техполя", Icon: Settings },
];
...
function sheetXml(sheet) {
  ...
  const autoFilterRow = Number(sheet.autoFilterRow || 1);
  const autoFilterStart = Math.max(1, Math.min(autoFilterRow, rows.length || 1));
  const autoFilter = rows.length > 1 && sheet.autoFilter !== false
    ? `<autoFilter ref="A${autoFilterStart}:${columnName(maxColumns - 1)}${Math.max(rows.length, autoFilterStart)}"/>`
    : "";
...
const sellerReportTemplates = [
  {
    ...
    format: "XLSX",
    description: "Рабочий отчет по воронке, рекламе, акциям, остаткам, рекомендациям и источникам данных.",
  },
];
...
function reportHeaderRow(values) {
  return values.map((value) => ({ value, style: 1 }));
}

function reportUniqueJoin(values, separator = ", ") {
  const seen = new Set();
  const items = [];
...
function reportValueFromKeys(object, keys, fallback = "") {
  const value = reportMetric(object, keys);
...
function reportAdMetricsForCard(reportData, period, card) {
      ...
      for (const app of day?.apps || []) {
        for (const nm of app?.nms || []) {
          if (String(nm?.nmId || "") === nmKey) {
            ...
            metrics.campaigns.push(firstDefined(advert?.name, advert?.advertName, advert?.advertId));
            metrics.statuses.push(firstDefined(advert?.status, advert?.statusName));
            metrics.placements.push(firstDefined(app?.name, app?.appType, app?.type));
  ...
  metrics.drr = metrics.orderSum ? (metrics.spend / metrics.orderSum) * 100 : "";
  metrics.campaignsText = reportUniqueJoin(metrics.campaigns);
  metrics.statusesText = reportUniqueJoin(metrics.statuses);
  metrics.placementsText = reportUniqueJoin(metrics.placements);
  return metrics;
}
...
function reportPromotionPlanPriceForCard(reportData, card) {
  const entry = reportPromotionEntriesForCard(reportData, card, false)
    .find((item) => firstDefined(item?.planPrice, item?.actionPrice, item?.promoPrice) !== "");
  return entry ? reportPrice(firstDefined(entry?.planPrice, entry?.actionPrice, entry?.promoPrice)) : "";
}

function reportPromotionStatusForCard(reportData, card) {
  const entries = reportPromotionEntriesForCard(reportData, card, false);
  if (!entries.length) {
    return "Не участвует";
  }
  if (entries.some((item) => Boolean(item?.inAction))) {
    return "Участвует";
  }
  return "Доступна к добавлению";
}

function reportAutoRecommendation({ currentOrders, pastOrders, daysToStockEnd, adMetrics, promotionStatus }) {
  const notes = [];
  const current = Number(currentOrders);
  const past = Number(pastOrders);
  const days = Number(daysToStockEnd);
  if (Number.isFinite(current) && Number.isFinite(past)) {
    if (past > 0 && current < past) {
      notes.push(`Заказы снизились: ${current} против ${past}.`);
    } else if (current > past) {
      notes.push(`Заказы выросли: ${current} против ${past}.`);
    }
  }
  if (Number.isFinite(days) && days > 0 && days < 14) {
    notes.push(`Остатка примерно на ${reportNumber(days, 1)} дн.; стоит проверить поставку.`);
  }
  if (Number(adMetrics?.spend || 0) > 0 && Number(adMetrics?.orderSum || 0) === 0) {
    notes.push("Есть расход РК без суммы заказов; проверить эффективность кампании.");
  } else if (Number(adMetrics?.drr || 0) > 30) {
    notes.push(`ДРР РК ${reportPercent(adMetrics.drr)}%; нужна проверка ставки и товаров в кампании.`);
  }
  if (promotionStatus === "Доступна к добавлению") {
    notes.push("Есть доступная акция WB; проверить цену входа и маржинальность.");
  }
  return notes.join(" ") || "Данные собраны; решение можно отметить после проверки специалистом.";
}
...
function reportWeeklyRows(reportData, portal, cards, period, periods) {
  ...
  return [
    ...
    ...cards.map((card) => {
      ...
      const impressions = reportValueFromKeys(selected, ["viewCount", "views", "impressions", "openCard", "showCount"]);
      const openCount = reportValueFromKeys(selected, ["openCount", "openCardCount", "clickCount", "openCard"]);
      const avgOrdersPerDay = firstDefined(selected.avgOrdersCountPerDay, orders.ordersCount ? Number((Number(orders.ordersCount) / periodDays).toFixed(2)) : "");
      const stock = firstDefined(
        selected.stocks,
        selected.stock,
        Number(selected.stockWb || 0) + Number(selected.stockMp || 0) || "",
        cardReportStock(card),
      );
      const daysToStockEnd = stock && avgOrdersPerDay ? reportNumber(Number(stock) / Number(avgOrdersPerDay), 1) : "";
      const promotionStatus = reportPromotionStatusForCard(reportData, card);
      return [
        ...
        reportNumber(impressions),
        reportNumber(openCount),
        reportPercent(reportValueFromKeys(selected, ["ctr", "ctrPercent"])),
        ...
        adMetrics.campaignsText,
        adMetrics.statusesText,
        reportNumber(adMetrics.orders),
        reportPrice(adMetrics.spend),
        reportPrice(adMetrics.orderSum),
        reportPercent(adMetrics.drr),
        promotionStatus,
        reportPrice(cardReportPrice(card)),
        reportPromotionPriceForCard(reportData, card),
        reportPromotionPlanPriceForCard(reportData, card),
        "",
        "",
        reportAutoRecommendation({ currentOrders, pastOrders, daysToStockEnd, adMetrics, promotionStatus }),
        "",
        "",
...
function reportInstructionRows() {
  return [
    ["ИНСТРУКЦИЯ ПО ЕЖЕНЕДЕЛЬНОМУ ОБНОВЛЕНИЮ ОТЧЕТА WB"],
    ["Файл формируется автоматически из OptiCards и подключенных WB API. Основной рабочий лист с датой отчета уже содержит собранные показатели и служебные колонки для решений."],
    [],
    reportHeaderRow(["Лист", "Что внутри", "Источник", "Как используется"]),
    ["Лист с датой", "Основной клиентский отчет", "Карточки WB, воронка, реклама, акции", "Специалист смотрит показатели, рекомендации и заполняет решения/отметки"],
    ["WB_Воронка_текущая", "Показатели воронки за выбранный период", "WB Analytics sales-funnel/products", "Показы, переходы, CTR, корзина, заказы, сумма заказов, остаток, доставка, локальность"],
    ["WB_Воронка_прошлая", "Сравнимый прошлый период", "WB Analytics pastPeriod или предыдущая неделя отчета", "Расчет динамики заказов"],
    ["WB_Цены_загрузка", "Текущие цены и остатки из карточек", "OptiCards snapshot + WB данные кабинета", "Текущая цена товара и резервные остатки"],
    ["WB_РК_API", "Статистика рекламы по артикулам", "WB Promotion promotion/count + fullstats", "Наличие РК, расход, заказы и ДРР"],
    ["WB_Акции_загрузка", "Текущее участие и доступные акции", "WB Prices and Discounts calendar/promotions", "Участие сейчас и цена участия"],
    ["WB_Акции_план", "Плановые поля по акциям", "Автоматически подготовленный лист", "Можно отметить план участия и плановую цену"],
    ["Источники", "Статус каждого API-блока", "Ответ backend", "Быстрая диагностика, почему часть колонок может быть пустой"],
    [],
    ["Что заполняется вручную"],
    ["В основном листе вручную заполняются: Планируем участвовать, Планируемая цена, Предложение клиента, Отметка о выполнении, Комментарии клиента."],
    ["Колонка Предложение ВебОптимайз заполняется автоматической подсказкой и остается editable для специалиста."],
  ];
}

function reportFunnelRows(reportData, cards, period, mode = "selected") {
  const header = [
    "Артикул продавца",
    "Артикул WB",
    "Название",
  ...
  const rows = [reportHeaderRow(header)];
  for (const card of cards) {
    const analytics = reportAnalyticsEntry(reportData, period, card);
    const item = mode === "past"
      ? (String(analytics.sourceRole || "").startsWith("pastPeriod") ? analytics.selected || {} : analytics.past || analytics.selected || {})
      : analytics.selected || {};
    rows.push([
      card?.vendorCode || "",
      reportNmKey(card),
      card?.title || "",
      card?.subjectName || "",
      reportNumber(reportValueFromKeys(item, ["viewCount", "views", "impressions", "openCard", "showCount"])),
      reportPercent(reportValueFromKeys(item, ["ctr", "ctrPercent"])),
      reportNumber(reportValueFromKeys(item, ["openCount", "openCardCount", "clickCount", "openCard"])),
      reportNumber(firstDefined(item.cartCount, item.addToCartCount)),
      reportNumber(firstDefined(item.orderCount, item.ordersCount)),
      reportPrice(firstDefined(item.orderSum, item.ordersSum)),
      reportNumber(item.avgOrdersCountPerDay, 2),
      reportNumber(firstDefined(item.stockWb, item.stocksWb, item.wbStock)),
      reportNumber(firstDefined(item.stockMp, item.stocksMp, item.sellerStock)),
      reportNumber(firstDefined(item.stocks, item.stock, cardReportStock(card))),
      reportReadyTime(item.timeToReady),
      reportPercent(item.localizationPercent),
    ]);
  }
  if (rows.length === 1) {
    rows.push(["нет данных", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]);
...
function reportPriceUploadRows(cards) {
  const rows = [reportHeaderRow(["Бренд", "Категория", "Артикул WB", "Артикул продавца", "Текущая цена", "Остатки WB", "Остатки продавца", "Цена со скидкой"])];
  for (const card of cards) {
    rows.push([
      card?.brand || "",
      card?.subjectName || "",
      reportNmKey(card),
      card?.vendorCode || "",
      reportPrice(firstDefined(card?.price, card?.rawFields?.price)),
      reportNumber(firstDefined(card?.wbStock, card?.rawFields?.wbStock)),
      reportNumber(firstDefined(card?.sellerStock, card?.rawFields?.sellerStock)),
      reportPrice(cardReportPrice(card)),
    ]);
  }
  if (rows.length === 1) {
...
function reportSourceRows(reportData, cards, portal) {
  return [
    reportHeaderRow(["Блок", "Статус", "Источник", "Комментарий", "Строк"]),
    ...clientReportAvailability(cards, portal, reportData).map(([label, status, source]) => {
      const [sourceName, ...rest] = String(source || "").split(" · ");
      const rowsPart = rest.find((part) => part.startsWith("строк:"));
      return [label, status, sourceName || "", rest.filter((part) => !part.startsWith("строк:")).join(" · "), rowsPart?.replace("строк:", "").trim() || ""];
    }),
  ];
}

function reportAdRows(reportData) {
  const rows = [reportHeaderRow(["Период с", "Период по", "ID РК", "Название РК", "Статус РК", "Активна сейчас", "Тип оплаты", "Тип ставки", "Размещение", "Артикул WB", "Товар", "Показы", "Клики", "CTR", "Добавления в корзину", "Заказы", "Расход", "Сумма заказов", "ДРР"])];
  for (const advert of reportData?.ads?.stats || []) {
    for (const day of advert?.days || []) {
      for (const app of day?.apps || []) {
        for (const nm of app?.nms || []) {
          const orderSum = Number(nm?.sum_price || 0);
          const spend = Number(nm?.sum || 0);
          rows.push([
            String(day?.date || "").slice(0, 10),
            String(day?.date || "").slice(0, 10),
            advert?.advertId || "",
            firstDefined(advert?.name, advert?.advertName, advert?.advertId),
            firstDefined(advert?.statusName, advert?.status),
            "",
            firstDefined(advert?.paymentType, advert?.type),
            firstDefined(advert?.bidType, advert?.strategy),
            firstDefined(app?.name, app?.appType, app?.type),
            nm?.nmId || "",
            nm?.name || "",
            ...
            reportPrice(spend),
            reportPrice(orderSum),
            orderSum ? reportPercent((spend / orderSum) * 100) : "",
          ]);
        }
      }
    }
...
function reportPromotionPlanRows(reportData, cards) {
  ...
  const rows = [reportHeaderRow([
    ...
    "Плановая цена WB",
    "Планируем участвовать",
    "Наша планируемая цена",
    "Комментарий",
  ])];
  for (const card of cards) {
    const entries = reportPromotionEntriesForCard(reportData, card, false);
    const entriesForRow = entries.length ? entries : [{}];
    for (const item of entriesForRow) {
      const key = `${reportNmKey(card)}:${reportPromotionName(item)}`;
      if (activePlans.has(key)) {
        continue;
      }
      activePlans.add(key);
      rows.push([
        reportPromotionName(item) || "нет доступной акции",
        card?.vendorCode || item?.vendorCode || "",
        reportNmKey(card),
        card?.subjectName || item?.subjectName || item?.subject || "",
        card?.title || item?.name || item?.title || "",
        reportPrice(cardReportPrice(card)),
        reportPrice(firstDefined(item?.planPrice, item?.actionPrice, item?.promoPrice)),
        "",
        "",
        "",
      ]);
...
function buildClientWbReportSheets(portal, cards, reportData = null) {
  ...
  const weeklySheets = periods.map((period) => {
    const rows = reportWeeklyRows(reportPayload, portal, reportCards, period, periods);
    return {
      name: period.label || "Период",
      freezeRows: 5,
      autoFilterRow: 5,
      widths: [22, 16, 20, 18, 16, 14, 18, 12, 18, 20, 18, 18, 18, 24, 24, 18, 16, 26, 16, 30, 18, 20, 16, 24, 18, 24, 18, 22, 18, 20, 18, 48, 28, 24, 42, 32],
      dataValidations: rows.length > 5 ? [
        { range: `AD6:AD${rows.length}`, formula1: "\"да,нет,нужно проверить\"" },
        { range: `AH6:AH${rows.length}`, formula1: "\"не начато,в работе,готово,не актуально\"" },
      ] : [],
      rows,
    };
  });
  const latestPeriod = periods[0] || {};
  const previousPeriod = reportPreviousPeriod(periods, latestPeriod) || latestPeriod;
  return [
    ...weeklySheets,
    { name: "00_ИНСТРУКЦИЯ_WB", freezeRows: 4, autoFilterRow: 4, widths: [26, 58, 38, 70], rows: reportInstructionRows() },
    { name: "WB_Воронка_текущая", freezeRows: 1, widths: [22, 18, 54, 24, 14, 12, 20, 18, 20, 20, 26, 18, 20, 18, 20, 18], rows: reportFunnelRows(reportPayload, reportCards, latestPeriod, "selected") },
    { name: "WB_Воронка_прошлая", freezeRows: 1, widths: [22, 18, 54, 24, 14, 12, 20, 18, 20, 20, 26, 18, 20, 18, 20, 18], rows: reportFunnelRows(reportPayload, reportCards, previousPeriod, "past") },
    { name: "WB_Цены_загрузка", freezeRows: 1, widths: [18, 24, 18, 22, 16, 16, 18, 18], rows: reportPriceUploadRows(reportCards) },
    { name: "WB_РК_API", freezeRows: 1, widths: [14, 14, 16, 34, 18, 16, 18, 18, 18, 18, 48, 14, 14, 12, 18, 14, 16, 22, 14], rows: reportAdRows(reportPayload) },
    { name: "WB_Акции_загрузка", freezeRows: 1, widths: [42, 22, 18, 24, 52, 18, 22, 18, 22, 22, 48, 42], rows: reportPromotionRows(reportPayload, reportCards) },
    { name: "WB_Акции_план", freezeRows: 1, widths: [42, 22, 18, 24, 52, 18, 22, 20, 22, 42], rows: reportPromotionPlanRows(reportPayload, reportCards) },
    { name: "Остатки", freezeRows: 1, widths: [22, 18, 56, 22, 24, 22], rows: reportStockRows(reportPayload, reportCards, periods) },
    { name: "Источники", freezeRows: 1, widths: [28, 16, 42, 80, 12], rows: reportSourceRows(reportPayload, reportCards, portal) },
  ];
}
...
function WorkspaceTabs({ items, active, onChange, className = "seller-tabs", ariaLabel = "Разделы" }) {
  return (
    <nav className={className} aria-label={ariaLabel}>
      {items.map(({ key, label, Icon, badge, disabled = false, title = "" }) => (
        <button className={active === key ? "active" : ""} type="button" onClick={() => onChange(key)} disabled={disabled} title={title || label} key={key}>
...
function SellerCommandCenter({ marketplace = "WB", cardsCount = 0, issueCount = 0, activeTaskCount = 0, submittedCount = 0, sourceStatus = "", onOpenTasks, onOpenReports, onOpenWorkPeriods }) {
  const hasTasks = Number(activeTaskCount || 0) > 0;
  const hasReview = Number(submittedCount || 0) > 0;
  const hasIssues = Number(issueCount || 0) > 0;
  const focusText = hasReview
    ? "Есть карточки на согласовании"
    : hasTasks
      ? "Есть активная пачка задач"
      : hasIssues
        ? "Есть карточки к проверке"
        : "Критичных сигналов нет";
  const focusTone = hasReview ? "amber" : hasTasks ? "blue" : hasIssues ? "amber" : "green";
  return (
    <section className="workspace-strip command-center">
      <div className="strip-head">
        <div>
          <h2>Рабочий пульт</h2>
          <p>{marketplace}: быстрые переходы к тому, что требует действия.</p>
        </div>
        <Tag tone={focusTone}>{focusText}</Tag>
      </div>
      <div className="command-grid">
        <button className={`command-card ${hasTasks ? "hot" : ""}`} type="button" onClick={onOpenTasks}>
          <ClipboardList size={19} />
          <span>Задачи</span>
          <strong>{formatNumber(activeTaskCount)}</strong>
          <em>{hasTasks ? "продолжить пачку" : "создать работу из карточек"}</em>
        </button>
        <button className={`command-card ${hasReview ? "hot" : ""}`} type="button" onClick={onOpenTasks}>
          <CheckSquare size={19} />
          <span>Согласование</span>
          <strong>{formatNumber(submittedCount)}</strong>
          <em>{hasReview ? "проверить ожидание" : "пока пусто"}</em>
        </button>
        <button className={`command-card ${hasIssues ? "hot" : ""}`} type="button">
          <AlertTriangle size={19} />
          <span>К проверке</span>
          <strong>{formatNumber(issueCount)}</strong>
          <em>{hasIssues ? "открыть карточки ниже" : "без срочных сигналов"}</em>
        </button>
        <button className="command-card" type="button" onClick={onOpenReports}>
          <FileText size={19} />
          <span>Отчеты</span>
          <strong>{formatNumber(cardsCount)}</strong>
          <em>сформировать XLSX</em>
        </button>
        <button className="command-card" type="button" onClick={onOpenWorkPeriods}>
          <CalendarDays size={19} />
          <span>Период</span>
          <strong>план</strong>
          <em>работы отдела</em>
        </button>
        <div className="command-card passive">
          <Store size={19} />
          <span>Источник</span>
          <strong>{sourceStatus || "не задан"}</strong>
          <em>в настройках кабинета</em>
        </div>
      </div>
    </section>
  );
...
function CardPassportPanel({ marketplace = "WB", title = "", idLabel = "ID", idValue = "", articleLabel = "Артикул", articleValue = "", category = "", brand = "", source = "", photoCount = 0, characteristicCount = 0, issueCount = 0, statusLabel = "", statusTone = "blue" }) {
  return (
    <section className="workspace-strip card-passport-panel">
      <div className="strip-head">
        <div>
          <h2>Паспорт карточки</h2>
          <p>{marketplace}: базовые признаки, по которым специалист сверяет товар перед СЯ, аудитом и изменениями.</p>
        </div>
        <Tag tone={statusTone}>{statusLabel || source || marketplace}</Tag>
      </div>
      <div className="card-passport-grid">
        <div className="card-passport-title">
          <span>Название</span>
          <strong>{title || "Без названия"}</strong>
        </div>
        <div><span>{idLabel}</span><strong>{valueSummary(idValue)}</strong></div>
        <div><span>{articleLabel}</span><strong>{valueSummary(articleValue)}</strong></div>
        <div><span>Категория</span><strong>{valueSummary(category)}</strong></div>
        <div><span>Бренд</span><strong>{valueSummary(brand)}</strong></div>
        <div><span>Фото</span><strong>{formatNumber(photoCount)}</strong></div>
        <div><span>Характеристики</span><strong>{formatNumber(characteristicCount)}</strong></div>
        <div><span>К проверке</span><strong>{formatNumber(issueCount)}</strong></div>
        <div><span>Источник</span><strong>{source || marketplace}</strong></div>
      </div>
    </section>
  );
}
...
function OzonSellerScreen({ portal, displayUsers, findUser, canManage = false, onBack, sellerTab = "cabinet", onSellerTabChange, onUpdateTeam, onUpdateName, onUpdateSource, onPortalUpdated, onOpenCard, onNotice, helpEnabled = false }) {
  ...
  const ozonTasks = Array.isArray(ozonWorkState.tasks) ? ozonWorkState.tasks : [];
  const ozonActiveTasks = ozonTasks.filter((task) => !["approved", "done", "skipped"].includes(task.status)).length;
  const ozonSubmittedTasks = ozonTasks.filter((task) => task.status === "submitted").length;
      ...
      <div className="content">
        <div className="seller-layout">
          <div className="seller-main">
            <WorkspaceTabs items={sellerWorkspaceTabs} active={activeSellerTab} onChange={setSellerTab} ariaLabel="Разделы Ozon-кабинета" />
            ...
            {canManage ? (
              <details className="cabinet-management" open={nameEditing || undefined}>
                ...
                <div className="cabinet-management-body">
                  ...
                  <details className="management-subpanel">
                    <summary><Store size={16} />Источник данных</summary>
                    <div className="management-subpanel-body">
                      <p>{sourceDescription}</p>
                      {sourceEditing ? (
                        <div className="ozon-source-editor">
                          <label className="field-label">
                            Ссылка, Seller ID или ориентир Ozon
                            <input value={sourceDraft.storeUrl} onChange={(event) => updateSourceDraft("storeUrl", event.target.value)} placeholder="https://www.ozon.ru/seller/... или Seller ID" disabled={sourceSaving} />
                          </label>
                          <label className="field-label">
                            Что есть на старте
                            <textarea value={sourceDraft.manualSource} onChange={(event) => updateSourceDraft("manualSource", event.target.value)} placeholder="Например: список SKU, ссылка на витрину, файл клиента или комментарий по тестовой пачке." disabled={sourceSaving} />
                          </label>
                  ...
                  <details className="management-subpanel">
                    <summary><Settings size={16} />Контур Ozon</summary>
                ...
                <section className="workspace-strip">
                  <div className="strip-head">
                    <div>
                      ...
                      <p>Фактическое состояние источника данных, карточек и задач.</p>
                    </div>
                    <Tag tone="amber">beta</Tag>
                  </div>
                  <div className="summary-grid">
                    ...
                    <Metric label="Активные задачи" value={formatNumber(ozonActiveTasks)} />
                    <Metric label="На согласовании" value={formatNumber(ozonSubmittedTasks)} />
                  </div>
                </section>

                <SellerCommandCenter
                  marketplace="Ozon"
                  cardsCount={portal.cardCount}
                  issueCount={portal.problemCount}
                  activeTaskCount={ozonActiveTasks}
                  submittedCount={ozonSubmittedTasks}
                  sourceStatus={sourceConfigured ? "MPStats" : "не задан"}
                  onOpenTasks={() => setSellerTab("tasks")}
                  onOpenReports={() => setSellerTab("reports")}
                  onOpenWorkPeriods={() => setSellerTab("work-periods")}
                />
      ...
      <div className="content">
        <div className={`detail-layout ${activeTab === "technical" ? "wide-changes" : ""}`}>
          ...
          <div className="detail-main">
            <WorkspaceTabs items={ozonCardWorkspaceTabs} active={activeTab} onChange={setActiveTab} className="detail-tabs" ariaLabel="Разделы Ozon-карточки" />
            ...
            {activeTab === "card" ? (
              <>
                <CardPassportPanel
                  ...
                  idLabel="SKU"
                  idValue={sku}
                  articleLabel="Offer ID"
                  articleValue={offerId}
                  category={category}
                  brand={brand}
                  source={sourceLabel}
                  photoCount={Array.isArray(photos) ? photos.length : 0}
                  characteristicCount={ozonCardCharacteristicItems(card).length}
            ...
            {activeTab === "competitors" ? (
              <OzonCompetitorAuditPanel card={card} portal={portal} draft={ozonCardDraft} status={ozonCardDraftStatus} />
            ) : null}
  ...
  return (
    <section className="workspace-strip">
      ...
      <div className="commerce-summary">
        ...
        <div><span>Описание</span><strong>{descriptionDraft.trim() ? `${descriptionDraft.length} симв.` : "пусто"}</strong></div>
        <div><span>Статус</span><strong>{approvalMeta.label}</strong></div>
        <div><span>Источник</span><strong>Ozon draft</strong></div>
      </div>
...
function OzonCompetitorAuditPanel({ card, portal, draft, status = "idle" }) {
  const rawFields = rawFieldsForCard(card);
  const { sku, offerId } = ozonCardIdentity(card);
  const title = card?.title || rawFields.title || rawFields.name || "Ozon карточка";
  const category = ozonCardCategory(card);
  const photoCount = ozonCardPhotoCount(card);
  const characteristicCount = ozonCardCharacteristicItems(card).length;
  const auditResult = draft?.draft?.auditResult || null;
  const hasDraft = Boolean(draft?.draft?.content?.title || draft?.draft?.content?.description);
  const sourceLabel = rawFields.source || card?.source || "Ozon snapshot";
  const flow = [
    { title: "Карточка", copy: status === "loading" ? "загружаем сохраненный снимок" : "данные карточки доступны", status: status === "loading" ? "active" : "done" },
    { title: "Рыночный аудит", copy: auditResult ? "есть базовые выводы" : "можно запустить на соседней вкладке", status: auditResult ? "done" : "" },
    { title: "Конкуренты Ozon", copy: "подбор и сравнение подключаются отдельным слоем", status: "active" },
  ];
  ...
  return (
    <section className="workspace-strip audit-workspace">
      <div className="strip-head">
        <div>
          <h2>Товарный аудит Ozon</h2>
          <p>Единый экран для будущего сравнения с конкурентами Ozon: карточка, базовый аудит и контекст уже собраны рядом.</p>
        </div>
        <Tag tone="amber">следующий слой</Tag>
      </div>
      <div className="audit-flow">
        {flow.map((step) => (
          <div className={`audit-flow-step ${step.status}`} key={step.title}>
      ...
      <div className="audit-next-step">
        <div>
          <span>Что уже видно</span>
          <strong>{title}</strong>
          <p>SKU {sku} · offer {offerId} · {category}. После подключения Ozon-конкурентов этот экран станет парой к WB-товарному аудиту.</p>
        </div>
      </div>
      <div className="commerce-summary">
        <div><span>Фото</span><strong>{formatNumber(photoCount)}</strong></div>
        <div><span>Характеристики</span><strong>{formatNumber(characteristicCount)}</strong></div>
        <div><span>Рыночный аудит</span><strong>{auditResult ? "готов" : "нет"}</strong></div>
        <div><span>Черновик</span><strong>{hasDraft ? "есть" : "пусто"}</strong></div>
        <div><span>Источник</span><strong>{sourceLabel}</strong></div>
        <div><span>Кабинет</span><strong>{portalDisplayName(portal)}</strong></div>
      </div>
      <div className="problem-reasons">
        <Tag tone="blue">без WB API</Tag>
        <Tag tone="amber">ждет источник конкурентов Ozon</Tag>
      </div>
    </section>
  );
}
...
function SellerScreen({ portal, cards, cardsLoading = false, mpstatsIntegration = null, displayUsers, findUser, canManage = false, onBack, onOpenCard, sellerTab = "cabinet", onSellerTabChange, onOpenModal, onRefreshCards, onResetWork, onUpdateTeam, onUpdateName, onPortalUpdated, onNotice, helpEnabled = false }) {
  ...
  const sellerActiveTaskCount = (Array.isArray(approvalWorkflow.tasks) && approvalWorkflow.tasks.length)
    ? approvalWorkflow.tasks.length
    : portalActiveTaskCount(portal);
  const sellerSubmittedTaskCount = Number(approvalWorkflow.analytics?.pendingCount || portal.draftSummary?.approvalPendingCount || 0);
  ...
  return (
    <section className="screen active marketplace-theme-wildberries">
      ...
      <div className="content">
        <div className="seller-layout">
          <div className="seller-main">
            <WorkspaceTabs items={sellerWorkspaceTabs} active={activeSellerTab} onChange={setSellerTab} ariaLabel="Разделы WB-кабинета" />
            ...
            {canManage ? (
              <details className="cabinet-management" open={nameEditing || undefined}>
                ...
                <div className="cabinet-management-body">
                  ...
                  <details className="management-subpanel">
                    ...
                    <div className="management-subpanel-body">
                      <p>{portal.syncStatus === "loaded"
                        ? "Список карточек загружен из WB API через backend. Запись в WB включается отдельно."
                        : (isMpstatsLoaded
                          ? "Кабинет заведен без WB API, карточки загружены через MPStats. API можно подключить позже."
                          : (isManual
                            ? "Кабинет заведен без WB API. Источник можно обновить через MPStats или подключить API."
                            : "Кабинет подключается для чтения данных."))}</p>
                      <div className="panel-actions">
                  ...
                  <details className="management-subpanel">
                    <summary><Settings size={16} />Контур безопасности</summary>
                    <div className="security-inline-list">
            ...
            <section className="workspace-strip">
              ...
              <div className="summary-grid">
                <Metric label="Карточек в кабинете" value={formatNumber(portal.cardCount)} />
                <Metric label="К проверке" value={formatNumber(portal.problemCount)} />
                <Metric label="Активные задачи" value={formatNumber(sellerActiveTaskCount)} />
                <Metric label="На согласовании" value={formatNumber(sellerSubmittedTaskCount)} />
              </div>
            </section>
            ...
            <SellerCommandCenter
              ...
              cardsCount={portal.cardCount}
              issueCount={portal.problemCount}
              activeTaskCount={sellerActiveTaskCount}
              submittedCount={sellerSubmittedTaskCount}
              sourceStatus={portal.apiConnected ? "WB API" : (isMpstatsLoaded ? "MPStats" : "без API")}
              onOpenTasks={() => setSellerTab("tasks")}
              onOpenReports={() => setSellerTab("reports")}
              onOpenWorkPeriods={() => setSellerTab("work-periods")}
            />
      ...
      <section className="workspace-strip reports-strip">
        ...
        <HelpList
          ...
          items={[
            ...
            "Выберите период С какого числа и По какое число.",
            "Нажмите Сформировать. Файл XLSX с основным листом периода и служебными вкладками WB скачается автоматически.",
          ]}
        />
        <div className="report-builder">
          <div className="report-template-list">
    ...
    <section className="screen active">
      ...
      <div className="content">
        <div className={`detail-layout ${activeTab === "changes" || activeTab === "competitors" ? "wide-changes" : ""}`}>
          ...
          <div className="detail-main">
            <WorkspaceTabs items={cardWorkspaceTabs} active={activeTab} onChange={switchDetailTab} className="detail-tabs" ariaLabel="Разделы WB-карточки" />
            ...
            {activeTab === "card" ? (
              <>
                <CardPassportPanel
                  ...
                  articleValue={card?.vendorCode}
                  category={card?.subjectName}
                  brand={card?.brand}
                  source={cardSourceLabel}
                  photoCount={valueCount(photos)}
                  characteristicCount={currentCardCharacteristicRows.length}
