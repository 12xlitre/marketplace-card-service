...
Приложение сейчас является статическим сайтом: основной файл `index.html`.
...
В проде статику отдает nginx-контейнер. Менять код приложения специально под это не нужно.
