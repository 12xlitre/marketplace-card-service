...
SEO expansion MPStats отправляется не только с пользовательским стартовым запросом, но и с релевантными seed-фразами из карточки, если они явно читаются в текущем контенте/характеристиках. Например для очков с описанием формы `кошки/кошачий глаз` backend добавляет `очки кошачий глаз`, чтобы такие высокочастотные запросы попадали в кандидаты к добавлению.
