...
`POST /api/card-content-reoptimize` переоптимизирует заголовок, описание и черновик характеристик через настроенный LLM/GigaChat по вкладке `Семантическое ядро`. Маршрут принимает `selectedKeywords`, `currentKeywords`, `removeKeywords`, опциональный `characteristicsContext` и опциональный `sections` (`title`, `description`, `characteristics`), а также добирает сохраненное СЯ из `card_drafts.meta.semanticCoreFinal/semanticCoreReports`: в цель попадают добавленные ключи, действующие ключи в контенте и ранжируемые ключи без запросов, отмеченных к удалению. Если `sections` передан, backend сохраняет и возвращает только выбранные блоки, не меняя значения/source/reason остальных блоков черновика. Заголовок строится по WB-формуле `Тип товара -> Назначение -> Целевая аудитория -> Главная характеристика -> Бренд -> Модель`: тип товара идет первым, бренд ближе к концу, цвет и артикул не вытесняют сильные поисковые признаки. Описание ограничено 2000 символами и получает целевой диапазон 1700-1950 символов: ключи распределяются по покупательским абзацам (`что это`, позиционирование модели, назначение и линзы, дизайн/цвет, материал/комплект, бренд/итог), а не вставляются механическим списком. Нормальный русский язык и фактическая точность важнее формального счетчика: неестественные запросы можно адаптировать или пропускать, цвет из нескольких значений описывается как сочетание цветов одной модели, комплект не связывается с посадкой, а оправа не описывается как самостоятельная коррекция зрения. `contentOptimization` считает использованные ключи по точному фразовому вхождению. Маршрут требует доступ пользователя к `portalId`, не пишет в WB, сохраняет результат в `card_drafts` через тот же контур прав и возвращает `draftContent`/`draft` для вкладки `Изменения`.
...
В СЯ прямой счетчик `wb_count`/`wbcount` из MPStats считается `частотой WB`; кластерные поля `wb_cluster_count`/`norm_query_count` сохраняются отдельно как `кластер WB` и не используются как fallback для частоты запроса.
...
`GET /api/portal-card-drafts` отдает сохраненные черновики карточек по одному кабинету после проверки доступа. Frontend использует его для кабинетных XLSX-выгрузок: итоговое СЯ по карточкам с сохраненным СЯ и итоговый контент по карточкам, где секция `Контент` отправлена на согласование или уже принята (`submitted/approved/exported`).
...
`GET/POST /api/portals/<portal_id>/ozon-tasks` хранит beta-задачи Ozon отдельно от WB workflow. Маршрут доступен только пользователям с доступом к Ozon-кабинету, проверяет `marketplace=Ozon`, пишет задачи в `ozon_tasks`, события в `ozon_task_events` и не использует WB `card_drafts`/`card-workset`. Payload задачи сохраняет Ozon batch-поля `batchId`, `batchTitle`, `batchPosition`, `batchCreatedAt`, `workTypes`, `workComment`; frontend группирует карточки в массовые пачки, привязывает пачку к `portal_work_periods` через общий `link_task` и оставляет `localStorage` только как fallback/миграцию старых beta-задач. Поддерживаемые статусы: `draft`, `submitted`, `approved`, `done`, `skipped`, `later`, `returned`.
...
`GET /api/portals/<portal_id>/wb-client-report` собирает данные для клиентского XLSX-отчета по выбранному периоду `start/end` в формате `YYYY-MM-DD`. Старый параметр `weeks` остается совместимым fallback, но UI использует сценарий `выбрать отчет -> выбрать период -> сформировать` во вкладке `Отчеты` внутри кабинета селлера. XLSX формируется как рабочий отчет: основной лист периода с воронкой, рекламой, акциями, остатком в днях, локальностью, динамикой заказов, автоматической подсказкой `Предложение ВебОптимайз` и ручными полями для решений, плюс служебные листы источников WB. Маршрут проверяет доступ к кабинету, берет WB-токен только из backend-хранилища и не выполняет write-операции в WB.
