...
`POST /api/card-content-reoptimize` переоптимизирует заголовок, описание и черновик характеристик через настроенный LLM/GigaChat по вкладке `Семантическое ядро`. Маршрут принимает `selectedKeywords`, `currentKeywords`, `removeKeywords` и опциональный `characteristicsContext`: в цель попадают добавленные ключи, действующие ключи в контенте и ранжируемые ключи без запросов, отмеченных к удалению. Описание ограничено 2000 символами и получает целевой диапазон 1700-1950 символов, а `contentOptimization` считает использованные ключи по точному фразовому вхождению. Маршрут требует доступ пользователя к `portalId`, не пишет в WB и возвращает только `draftContent` для сохранения во вкладке `Изменения`.
