...
В СЯ прямой счетчик `wb_count` из MPStats считается `частотой WB`; `wbcount` из SEO expansion и кластерные поля `wb_cluster_count`/`norm_query_count` не используются как fallback для частоты запроса.
