...
Ты — редактор карточек Wildberries для живого покупателя. SEO — это ограничение и источник словаря, но не стиль текста. Переоптимизируй заголовок, описание и черновик характеристик карточки под согласованное семантическое ядро так, чтобы карточку было приятно читать человеку.
...
- не пытайся любой ценой сохранить точную форму ключа: лучше естественная русская фраза и предупреждение, чем строка, похожая на SEO-склейку;
...
- не вставляй подряд перестановки одного и того же ключа: "солнцезащитные очки женские", "очки солнцезащитные женские", "женские солнцезащитные очки" должны раскрывать разные мысли или часть вариантов нужно пропустить;
- не начинай соседние предложения одинаково со слов "очки", "солнцезащитные очки", "солнечные очки", "женские/мужские очки"; если фраза нужна для SEO, впиши ее в середину предложения;
...
- перед финальным JSON сделай редакторский проход "human": убери одинаковый ритм предложений, канцелярит, рекламный пафос, принудительные списки из трех выгод, фразы "не просто..., а...", "идеально подходит", "станет отличным выбором", "создает стильный образ", "подчеркивает индивидуальность", если они не несут конкретный факт;
- предложения должны быть разной длины: часть короткие и ясные, часть чуть длиннее с пользой для покупателя; не делай весь текст одинаковыми SEO-блоками по 15-20 слов;
- каждый абзац должен отвечать на простой вопрос покупателя: что это, кому подойдет, как носить/использовать, какие факты проверить перед покупкой; не добавляй абзац только ради нового ключа;
- не используй "мы", "наш товар", "данная модель", "изделие является", "представляет собой", "обладает рядом преимуществ" и похожий канцелярит;
...
- бренд вписывай как часть названия, модели и покупательского контекста; не пиши мета-комментарии "бренд указан", "указан в карточке", "как бренд", "помогает найти по названию", "узнаваемый стиль бренда", если это не звучит как описание самого товара;
...
Верни строго JSON:
{
  ...
  "characteristics": [
    {"name": "...", "charcID": "", "values": ["..."], "reason": "...", "keywords": ["..."]}
  ],
  "_meta": {"warnings": ["..."]}
}
""".strip()
...
def content_reoptimization_target_keywords(selected, current, remove, limit=140):
  remove_keys = {content_keyword_normalized_key(item) for item in remove if content_keyword_normalized_key(item)}
  output = []
  seen = set()
  ...
  def add_rows(rows, source):
    for row in rows if isinstance(rows, list) else []:
      if not isinstance(row, dict):
        continue
      key = content_keyword_normalized_key(row)
      query = audit_str(row.get("query") or "", 120)
      if not key or key in seen or key in remove_keys or not query:
        continue
      seen.add(key)
      output.append({
        **row,
  ...
  add_rows(selected, "added")
  add_rows(current, "current")
  return output[:limit]
...
def content_contains_exact_keyword(text, query):
  text_tokens = content_exact_keyword_tokens(text)
  query_tokens = content_exact_keyword_tokens(query)
  if not text_tokens or not query_tokens or len(query_tokens) > len(text_tokens):
    return False
  for index in range(0, len(text_tokens) - len(query_tokens) + 1):
    if text_tokens[index:index + len(query_tokens)] == query_tokens:
      return True
  return False


def content_keyword_matches(text, keywords, limit=140):
  output = []
  for item in keywords if isinstance(keywords, list) else []:
    query = item.get("query") if isinstance(item, dict) else item
    if query and content_contains_exact_keyword(text, query):
      output.append(audit_str(query, 120))
    if len(output) >= limit:
      break
  return output


def content_characteristic_draft_key(name, charc_id=None):
  if charc_id:
    return f"charc:{charc_id}"
  normalized = audit_normalized(name)
  return f"charc-name:{normalized}" if normalized else ""


def content_characteristic_values(value, limit=8):
  if value is None:
    return []
  if isinstance(value, list):
    output = []
    for item in value:
      if isinstance(item, dict):
        output.extend(content_characteristic_values(item.get("value") or item.get("name"), limit=limit))
      else:
        output.extend(content_characteristic_values(item, limit=limit))
      if len(output) >= limit:
        break
    return audit_unique(output, limit=limit)
  if isinstance(value, dict):
    return content_characteristic_values(value.get("values") or value.get("value") or value.get("name"), limit=limit)
...
def normalize_content_reoptimization_characteristics(value, fallback_rows=None, limit=24):
  ...
  if isinstance(value, dict):
    for key, item in value.items():
      if isinstance(item, dict):
        rows.append({**item, "_fallbackKey": key})
  elif isinstance(value, list):
    rows = [item for item in value if isinstance(item, dict)]
  output = {}
  fallback_rows = fallback_rows if isinstance(fallback_rows, list) else []
  ...
  for item in rows[:limit]:
    ...
    if not name or not values:
      continue
    key = content_characteristic_draft_key(name, charc_id)
    if not key:
      continue
    ...
    row = {
      ...
      "source": "semantic",
      "reason": reason,
      "unitName": audit_str(item.get("unitName") or fallback.get("unitName") or ""),
...
def semantic_keyword_text(rows):
  return " ".join(item.get("query") or "" for item in rows if isinstance(item, dict))


def content_characteristic_name_from_context(preferred, characteristics, available):
  candidates = []
  for row in characteristics if isinstance(characteristics, list) else []:
    candidates.append((row.get("name") or "", row.get("charcId") or row.get("charcID") or "", row))
  for row in available if isinstance(available, list) else []:
    if isinstance(row, dict):
      candidates.append((row.get("name") or row.get("label") or "", row.get("charcID") or row.get("charcId") or "", row))
  preferred_tokens = set(audit_tokens(preferred))
  best = None
  best_score = 0
  for name, charc_id, source in candidates:
    tokens = set(audit_tokens(name))
    score = len(preferred_tokens & tokens)
    if audit_characteristic_name_score(preferred, name) > 0:
      score += 4
    if score > best_score:
      best_score = score
      best = (name, charc_id, source)
  return best if best_score else (preferred, "", {})


def content_reoptimization_characteristic_fallback(card, keyword_rows, available_characteristics=None, limit=16):
  card_rows = audit_card_characteristics(card)
  available_characteristics = available_characteristics if isinstance(available_characteristics, list) else []
  output = {}
  keyword_text = semantic_keyword_text(keyword_rows)
  keyword_tokens = set(semantic_content_tokens(keyword_text))

  for row in card_rows:
    values = row.get("values") or []
    if not values:
      continue
    row_text = " ".join([row.get("name") or "", *values])
    row_tokens = set(semantic_content_tokens(row_text))
    matched_queries = [
      item.get("query")
      for item in keyword_rows
      if isinstance(item, dict) and item.get("query") and (row_tokens & set(semantic_content_tokens(item.get("query"))))
    ][:5]
    ...
    output[key] = {
      ...
      "source": "semantic",
      "reason": (
        f"Текущее значение подходит под ключи: {', '.join(matched_queries[:3])}."
        if matched_queries else
        "Текущее значение связано с семантикой; проверьте, что оно заполнено в WB как отдельная характеристика."
      ),
  ...
  hints = []
  normalized_keywords = audit_normalized(keyword_text)
  if "женск" in normalized_keywords:
    hints.append(("Пол", ["женский"], "Ключи содержат женское позиционирование; если поле доступно в WB, заполните его отдельным значением."))
  if "мужск" in normalized_keywords:
    hints.append(("Пол", ["мужской"], "Ключи содержат мужское позиционирование; если поле доступно в WB, заполните его отдельным значением."))
  if "зрения" in normalized_keywords:
    hints.append(("Назначение", ["для зрения"], "Ключи содержат сценарий для зрения; используйте только если товар действительно предназначен для установки линз."))
  if "без линз" in normalized_keywords:
    hints.append(("Комплектация", ["оправа без линз"], "Ключи содержат уточнение без линз; проверьте комплектацию и не добавляйте, если линзы входят в товар."))
  if "овал" in normalized_keywords:
    hints.append(("Форма оправы", ["овальная"], "Ключи содержат форму оправы; заполните, если форма подтверждена фото и WB-справочником."))

  for name, values, reason in hints:
    target_name, charc_id, source = content_characteristic_name_from_context(name, card_rows, available_characteristics)
    key = content_characteristic_draft_key(target_name, charc_id)
    ...
    output[key] = {
      ...
      "source": "semantic",
      "reason": reason,
      "unitName": source.get("unitName") if isinstance(source, dict) else "",
      "maxCount": source.get("maxCount") if isinstance(source, dict) else None,
      "charcType": source.get("charcType") if isinstance(source, dict) else None,
    }
    if len(output) >= limit:
      break
  return output
...
def content_reoptimization_shape_for_description(keyword_rows, facts, product_kind=""):
  fact_values = content_reoptimization_fact_values(facts, ["Форма оправы", "Форма очков", "Форма линзы", "Форма линз", "Форма"])
  for value in fact_values:
    shape = content_reoptimization_shape_from_value(value)
    if shape:
      return shape
  ...
  for item in keyword_rows if isinstance(keyword_rows, list) else []:
    ...
    if not query or re.search(r"\d", query):
      continue
    if product_kind == "sunglasses" and not any(marker in query for marker in ("очк", "солнц", "солнеч", "поляр")):
      continue
    for label, markers in CONTENT_FRAME_TITLE_SHAPES:
      if not any(marker in query for marker in markers):
        continue
...
def content_reoptimization_has_explicit_uv_protection(facts):
  fact_text = audit_normalized(" ".join(
    " ".join(str(value or "") for value in values)
    for values in facts.values()
    if isinstance(values, list)
  ))
  return any(marker in fact_text for marker in ("уф", "uv", "ультрафиолет"))
...
def content_reoptimization_has_sunglasses_polarization(card, keyword_rows, facts):
  ...
  for name in ("Поляризация", "Поляризационные линзы", "Поляризованные линзы", "Тип солнцезащитных очков"):
    values = content_reoptimization_fact_values(facts, [name])
    if not values:
      continue
    value_text = audit_normalized(" ".join(values))
    if any(marker in value_text for marker in ("нет", "отсутств", "без", "false", "0")):
      continue
    if "поляр" in audit_normalized(name) or "поляр" in value_text or value_text in {"да", "есть", "true", "1"}:
      return True
...
def content_reoptimization_keyword_sentence(query, product_kind=""):
  ...
  if product_kind == "sunglasses":
    if "polaroid" in normalized or "полароид" in normalized or "палароид" in normalized or "поларойд" in normalized:
      return f"{clean_query[:1].upper() + clean_query[1:]} удобно носить в городе, за рулем и в отпуске."
    ...
    return f"{clean_query[:1].upper() + clean_query[1:]} подойдут покупателям, которым важны понятная посадка, спокойный стиль и защита от яркого света."
...
def content_reoptimization_preferred_query(keyword_rows, preferred=None, contains=None):
  ...
  contains = contains if isinstance(contains, list) else []
  by_key = {
    audit_normalized(item.get("query")): audit_str(item.get("query"), 120)
    for item in keyword_rows if isinstance(item, dict) and item.get("query")
  }
  for query in preferred:
    value = by_key.get(audit_normalized(query))
    if value:
      return value
  if contains:
    for item in keyword_rows:
      ...
      if query and all(part in normalized for part in contains):
        return query
  return ""


def content_reoptimization_preferred_query_filtered(keyword_rows, preferred=None, contains=None, blocked_markers=None):
  preferred = preferred if isinstance(preferred, list) else []
  contains = contains if isinstance(contains, list) else []
  blocked_markers = blocked_markers if isinstance(blocked_markers, tuple) else tuple(blocked_markers or ())

  def allowed(query):
    normalized = audit_normalized(query)
    return bool(query) and not any(marker in normalized for marker in blocked_markers)
  ...
  if contains:
    for item in keyword_rows if isinstance(keyword_rows, list) else []:
      query = audit_str(item.get("query") if isinstance(item, dict) else item, 120)
      normalized = audit_normalized(query)
      if query and all(part in normalized for part in contains) and allowed(query):
        return query
  return ""


def content_reoptimization_sunglasses_keyword_family(query):
  normalized = audit_normalized(query)
  tokens = set(content_exact_keyword_tokens(normalized))
  for marker, family in (
    ...
    ("от солнца", "sun"),
    ("солнеч", "sun"),
    ("солнцезащит", "sunglasses"),
  ):
    if marker in normalized:
      return family
  normalized_tokens = tokens - {"женские", "женская", "женск", "мужские", "мужская", "мужск", "очки", "очков", "очк"}
  return " ".join(sorted(normalized_tokens)) or normalized


def content_description_sentence_limit(text, max_chars=CONTENT_REOPTIMIZE_DESCRIPTION_LIMIT):
  text = audit_str(text)
...
def content_reoptimization_sunglasses_description(card, keyword_rows, facts, max_chars=CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MAX):
  ...
  has_uv_protection = content_reoptimization_has_explicit_uv_protection(facts)
  blocked_markers = tuple() if has_uv_protection else ("уф", "uv", "ультраф")

  def preferred(preferred_queries=None, contains=None):
    return content_reoptimization_preferred_query_filtered(
      keyword_rows,
      preferred=preferred_queries or [],
  ...
  brand_sentences = []
  brand_model_label = f"{brand_label} {model_name}".strip() if brand_label and model_name else brand_label
  brand_visual_details = []
  if shape:
    brand_visual_details.append(f"{shape} форма")
  if lens_color:
    brand_visual_details.append(f"цвет линз - {lens_color}")
  if color_values:
    if len(color_values) > 1:
      brand_visual_details.append(f"оправа в сочетании {color_combination} цветов")
    else:
      brand_visual_details.append(f"оправа цвета {color_combination}")
  if brand_model_label and brand_visual_details:
    brand_sentences.append(
      f"У модели {brand_model_label} главные визуальные детали - {'; '.join(brand_visual_details[:3])}."
    )
  elif brand_label:
    brand_sentences.append(f"Очки {brand_label} подойдут для повседневного ношения и поездок в яркую погоду.")
  brand_query = brand_sun_key or brand_reverse_key or brand_key
  if brand_query:
    brand_sentences.append(f"{cap(natural_query(brand_query))} выбирают по форме, оттенку линз и тому, как оправа смотрится на лице.")
  if brand_sentences:
  ...
  paragraphs.append(
    f"{final_subject} подходят как повседневные очки для солнца: для прогулок, дороги, отпуска и яркого дневного света. "
    "Фото, форма, размеры и комплект помогают заранее понять посадку и внешний вид модели."
  )
  ...
  used_families = {
    content_reoptimization_sunglasses_keyword_family(query)
    for query in (primary, everyday_key, sun_scenario_key, sun_gender_key, reverse, polar_main, polar_second, brand_query)
    if query
  }
  extra_keyword_sentences = 0
  for item in keyword_rows if isinstance(keyword_rows, list) else []:
    query = audit_str(item.get("query") if isinstance(item, dict) else item, 120)
    normalized = audit_normalized(query)
    if not query or content_contains_exact_keyword(text, query) or re.search(r"\d", normalized):
      continue
    if any(marker in normalized for marker in ("уф", "uv", "ультраф")) and not has_uv_protection:
      continue
    if any(marker in normalized and marker not in card_text for marker in blocked_without_fact):
      continue
    family = content_reoptimization_sunglasses_keyword_family(query)
    if family in used_families and family in {"sunglasses", "sun", "polarized", "brand", "shape", "uv"}:
      continue
    ...
    extra_keyword_sentences += 1
    if extra_keyword_sentences >= 5:
      break
    if len(text) >= CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MIN:
      break
  return content_description_sentence_limit(text, max_chars)
...
def content_reoptimization_frame_description(card, keyword_rows, facts, max_chars=CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MAX):
  ...
  vision_key = content_reoptimization_preferred_query(keyword_rows, preferred=["оправа для очков женская для зрения", "оправа женская для очков для зрения", "очки оправа для зрения"])
  no_lenses_key = content_reoptimization_preferred_query(keyword_rows, preferred=["оправа для очков женская без линз", "оправа для очков женская для зрения без линз", "очки без линз"])
  branded_key = content_reoptimization_preferred_query(keyword_rows, preferred=["оправа для очков женская для зрения брендовая"], contains=["брендов"])
  ...
  paragraphs = []
  positioning = "Модель рассчитана на повседневные и деловые образы: форма, цвет и посадка остаются главными ориентирами перед заказом."
  if colors:
    positioning = (
  ...
  if shape and colors:
    positioning = (
      f"Форма модели: {shape}. Сочетание {color_combination} цветов показывает, насколько аксессуар впишется в повседневный или деловой образ."
      if len(color_values) > 1 else
      f"Форма модели: {shape}. Цвет {colors} показывает, насколько аксессуар впишется в повседневный или деловой образ."
    )
  elif shape:
    positioning = f"Форма модели: {shape}; по ней можно оценить внешний вид оправы и понять, подойдет ли аксессуар к привычному стилю."

  paragraphs.append(
    f"{product_label} рассчитана на установку линз по рецепту и регулярное ношение. "
    f"{positioning}"
  )
  if frame_key:
    paragraphs.append(
  ...
  if under_key:
    paragraphs.append(
      f"{cap(under_key)} должна хорошо фиксировать линзы, удобно сидеть и совпадать с привычным стилем владельца; {model_reference} закрывает именно этот сценарий. "
  ...
  paragraphs.append(f"Оправа {final_model} подойдет, если нужно установить линзы по рецепту и получить аккуратный аксессуар для ежедневного ношения.")
...
def content_reoptimization_description_is_mechanical(text):
  ...
  awkward_phrases = (
    ...
    "указан в карточке как бренд",
    "указан в карточке",
    "бренд указан",
    "помогает найти по названию",
    "как бренд",
    "узнаваемый стиль бренда",
    "покупателю важно",
    "без лишних обещаний",
    ...
    "данное изделие",
    "наш товар",
    "подойдет для тех кто ценит",
    "для тех кто ценит",
    "позволяет подчеркнуть стиль",
  ...
  if len(short_keyword_sentences) >= 8:
    return True
  product_starts = 0
  for sentence in sentences:
    first_words = " ".join(sentence.split()[:3])
    if re.match(r"^(очки|очков|солнцезащитные очки|солнечные очки|женские очки|мужские очки|очки женские|очки мужские)\b", first_words):
      product_starts += 1
  repeated_starts = {}
  for sentence in sentences:
  ...
  if any(count >= 4 for count in repeated_starts.values()):
    return True
  semicolon_density = normalized.count(";")
  if len(sentences) >= 8 and semicolon_density >= 8:
    return True
  keyword_list_markers = sum(
    normalized.count(marker)
    for marker in (" для ", " с ", " и ")
  )
  average_sentence_length = sum(len(sentence.split()) for sentence in sentences) / max(1, len(sentences))
  if len(sentences) >= 10 and keyword_list_markers >= 45 and average_sentence_length < 17:
    return True
  return product_starts >= 5
...
def content_reoptimization_expand_description(description, card, target_keywords, min_chars=CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MIN, max_chars=CONTENT_REOPTIMIZE_DESCRIPTION_LIMIT):
  ...
  if is_frame_product:
    additions = [
      f"{title or subject} относится к категории {subject.lower()} и подходит для покупателей, которым нужна оправа для установки линз по рецепту.",
      f"Модель {brand or ''} рассчитана на ежедневное ношение: она фиксирует линзы, поддерживает комфортную посадку и сочетается с повседневной одеждой.".strip(),
    ]
  else:
    additions = [
  ...
  if is_frame_product:
    ...
    additions.extend([
      "Назначение, материалы, цвет, комплект и размеры лучше описывать рядом с реальными сценариями использования.",
      ...
      "Материалы, комплект и размеры лучше держать в отдельных фразах, чтобы описание читалось спокойно и без повторов.",
    ])
...
def normalize_mpstats_expanding_query(item):
  ...
  return {
    ...
    "wbCount": audit_int_from_first_present(item, ("wb_count", "wbCount"), 0),
    "wbExpansionCount": audit_int_from_first_present(item, ("wbcount",), 0),
    "wbClusterCount": audit_int_from_first_present(item, ("wb_cluster_count", "wbClusterCount", "norm_query_count", "normQueryCount"), 0),
...
def build_card_content_reoptimization(portal_id, card_key, raw_card, selected_keywords=None, current_keywords=None, remove_keywords=None, desired_keywords=None, draft=None, characteristics_context=None, sections=None):
  ...
  evidence = {
    ...
    "characteristicsContext": {
      ...
      "availableCharacteristics": available_characteristics[:120],
    ...
    "constraints": {
      "titleMaxChars": 60,
      "descriptionMaxChars": CONTENT_REOPTIMIZE_DESCRIPTION_LIMIT,
      "descriptionTargetMinChars": CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MIN,
      "descriptionTargetMaxChars": CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MAX,
      "language": "ru",
    },
  }
  messages = [
  ...
  initial_description_mechanical = content_reoptimization_description_is_mechanical(next_description)
  if target_keywords and (
    ...
    or initial_description_mechanical
  ):
    retry_messages = [
      *messages,
      {"role": "assistant", "content": content},
      {
        "role": "user",
        "content": json.dumps({
          "correction": "Результат не принят: описание слишком короткое, почти не содержит целевых ключей или звучит механически как SEO-текст.",
          "required": {
            ...
            "keywordSource": "evidenceBundle.semanticCore.allTargetKeywords",
            "naturalLanguagePriority": "нормальный русский текст важнее exact-вхождения кривого ключа",
            "style": "живое описание карточки для покупателя: разные начала предложений, конкретные факты, без SEO-склейки, канцелярита и рекламных штампов",
  ...
  used_keywords = content_keyword_matches(combined_text, target_keywords, limit=140)
  description_used_keywords = content_keyword_matches(next_description, target_keywords, limit=140)
  ...
  return {
    ...
    "contentOptimization": {
      ...
      "createdAt": utc_now().isoformat(),
      "engine": "opticards-semantic-content-v4.7-human",
      "provider": provider,
      "model": model,
      ...
      "descriptionUsedKeywords": description_used_keywords,
      "unusedKeywords": [item["query"] for item in target_keywords if item["query"] not in used_keywords][:140],
      "removeKeywordsStillPresent": remove_keywords_still_present,
      "titleLength": len(next_title),
      "descriptionLength": len(next_description),
      "descriptionTargetMinChars": CONTENT_REOPTIMIZE_DESCRIPTION_TARGET_MIN,
      "descriptionMaxChars": CONTENT_REOPTIMIZE_DESCRIPTION_LIMIT,
      "characteristics": len(characteristic_draft),
